...
public class ItemBloodbathBow extends ItemBow
{
    public ItemBloodbathBow()
    {
        this.maxStackSize = 1;
        ...
        this.addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter()
        {
            ...
            public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn)
            {
                ...
                {
                    return entityIn.getActiveItemStack().getItem() != ModItems.bloodbath_bow ? 0.0F : (stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) * DrawSpeed() / 20.0F;
                }
            }
        });
            ...
            if (!itemstack.isEmpty() || flag)
            {
                ...
                if ((double)f >= 0.1D)//if the velocity is lower than this, it does not fire
                {
                    ...
                    if (!worldIn.isRemote)
                    {
                        ...
                        if (f == 1.0F)
                        {
                        entityarrow.addTag("fired_by_bloodbath");
