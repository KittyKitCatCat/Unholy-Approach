...
public class ItemRingHoly extends Item implements IBauble
{
	...
	public static ItemStack isBaubleEquipped(EntityPlayer player)
	{
		...
		int slot = BaublesApi.isBaubleEquipped(player, ModItems.ring_holy);
		if (slot > 0)
		{
	...
	public static void damage(LivingHurtEvent event)
	{
		if (event.getSource().getTrueSource() != null)
		{
			if (event.getEntityLiving() instanceof EntityPlayer)
			{
				...
				{
					if (Math.random() < 0.25 || Math.random() < 0.5 && event.getSource() == ModDamageSources.thorns) {
